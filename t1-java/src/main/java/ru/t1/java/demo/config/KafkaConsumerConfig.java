...
public class KafkaConsumerConfig<T> {
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, AccountDto> accountKafkaListenerContainerFactory(@Qualifier("consumerAccountListenerFactory") ConsumerFactory<String, AccountDto> consumerFactory) {
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, TransactionDto> transactionKafkaListenerContainerFactory(@Qualifier("consumerTransactionListenerFactory") ConsumerFactory<String, TransactionDto> consumerFactory) {
    ...
    @Bean
    public ConsumerFactory<String, TransactionResultMessage> consumerTransactionResultListenerFactory() {
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, servers);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, groupId);
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, MessageDeserializer.class);
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, "ru.t1.java.demo.kafka.message.TransactionResultMessage");
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "*");
        props.put(JsonDeserializer.USE_TYPE_INFO_HEADERS, false);
        props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, sessionTimeout);
        props.put(ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG, maxPartitionFetchBytes);
        ...
        DefaultKafkaConsumerFactory factory = new DefaultKafkaConsumerFactory<String, TransactionResultMessage>(props);
        factory.setKeyDeserializer(new StringDeserializer());
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, TransactionResultMessage> transactionResultKafkaListenerContainerFactory
            (@Qualifier("consumerTransactionResultListenerFactory") ConsumerFactory<String, TransactionResultMessage> consumerFactory) {
